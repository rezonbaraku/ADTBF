...
/* ------------------- ADD YOUR METHODS BELOW THIS LINE ------------------- */
	public TreeNode getParent(TreeNode t) throws Exception {
		if(root.getItem() == t.getItem())
			return null;
		return getParent(root, t);
	}
	public TreeNode getParent(TreeNode maybeParent, TreeNode t) throws Exception {
		if(maybeParent.getLeft() != null) {
			if((int)maybeParent.getLeft().getItem() == (int)t.getItem()) {
				return maybeParent;
			}
			else if((int)maybeParent.getItem() > (int)t.getItem()) {
				return getParent(maybeParent.getLeft(), t);
			}
		}
		else if(maybeParent.getRight() != null) {
			if((int)maybeParent.getRight().getItem() == (int)t.getItem()) {
				return maybeParent;
			}
			else if((int)maybeParent.getItem() < (int)t.getItem()) {
				return getParent(maybeParent.getRight(), t);
			}
		}
		else{
			throw new Exception("Item not in the tree");
		}
		return t;
	}


	public TreeNode getParentt(TreeNode t) throws Exception {
	    if (root.getItem() == t.getItem()) {
	        return null; // If the given node is the root, it has no parent
	    }
	    ...
	    while (current != null) {
	        if (current.getLeft() != null && (int) current.getLeft().getItem() == (int) t.getItem()) {
	            return current; // If the left child's value matches t's value, current node is the parent
	        } else if (current.getRight() != null && (int) current.getRight().getItem() == (int) t.getItem()) {
	            return current; // If the right child's value matches t's value, current node is the parent
	        } else if ((int) current.getItem() > (int) t.getItem()) {
	    ...
	    throw new Exception("Item not in the tree"); // If the loop finishes without finding t, it means t is not in the tree
	}
